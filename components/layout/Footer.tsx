...
import { InstagramIcon, WhatsAppIcon, ScissorsIcon } from "@/components/ui/Icons";
      ...
      <Container className="grid gap-10 py-14 md:grid-cols-[1.2fr_1fr_1fr]">
        <div>
          ...
          <p className="mt-3 max-w-xs text-sm leading-relaxed text-[var(--color-ink-soft)]">
            ...
            acabamento fino e atendimento personalizado em {site.city}.
          </p>
        ...
        <div>
          <p className="text-sm font-medium text-[var(--color-ink)]">Navegação</p>
          <ul className="mt-4 space-y-2.5 text-sm text-[var(--color-ink-soft)]">
            <li><Link href="/" className="hover:text-[var(--color-brand)]">Início</Link></li>
            <li><Link href="/servicos" className="hover:text-[var(--color-brand)]">Serviços</Link></li>
            <li><Link href="/trabalhos" className="hover:text-[var(--color-brand)]">Trabalhos</Link></li>
            <li><Link href="/sobre" className="hover:text-[var(--color-brand)]">Sobre</Link></li>
          </ul>
        </div>

        <div>
          <p className="text-sm font-medium text-[var(--color-ink)]">Horários</p>
          <ul className="mt-4 space-y-2.5 text-sm text-[var(--color-ink-soft)]">
            {site.hours.map((h) => (
      ...
      <Container>
        <div className="flex flex-col items-center justify-between gap-3 border-t border-[var(--color-border)] py-6 text-xs text-[var(--color-ink-soft)] md:flex-row">
          <p className="flex items-center gap-2">
            ...
            © {new Date().getFullYear()} Sadina Santos. Todos os direitos reservados.
          </p>
          <p>{site.address}</p>
        </div>
      </Container>
